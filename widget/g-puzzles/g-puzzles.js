...
seajs.use(["jdf/1.0.0/unit/login/1.0.0/login.js",
	...
	if($('#content').attr('userlogin') == 1){
		puzzle_offside.attr('href','#anchor');
	}else{
		puzzle_offside.attr('href','#');
	}
	//默认状态
	...
	/*显示 关闭*/
	puzzle_offside.bind("click",function(){
		jdUser.login(function(){
			gDocW=$(document).width();
			gDocH=$(document).height();
			gOffset = $('.g_puzzle_box').offset();
			puzzle_offside.hide();
			if(window.location.href =="http://vip.jd.com/?pt=1"){
				window.location.href ="http://vip.jd.com/?pt=1#anchor";
			}
			loadAjax();
			return false;
		}, true);
	})
	if($('#content').attr('pt') == 1){
		puzzle_offside.trigger("click!");
	}
